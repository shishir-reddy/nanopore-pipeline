...
export WORKING_FOLDER="/fh/fast/radich_j/nanopore/Jobs/test_v9_Sat_Aug_25_15:14:18_PDT_2018"
...
if [ "$ALIGNER" = "last" ] || [ "$ALIGNER" = "both" ]; then
    ...
    # Last-train version
    last-train -Q1 -P6 -v $LASTAL_HUMANDB/humandb $DEMULTIPLEXED_DIRECTORY/none.fastq.gz >> $LAST_ALIGNMENT_DIRECTORY/last-parameters
    lastal -Q1 -P6 -vp $LAST_ALIGNMENT_DIRECTORY/last-parameters $LASTAL_HUMANDB/humandb $DEMULTIPLEXED_DIRECTORY/none.fastq.gz | last-split >> $LAST_ALIGNMENT_DIRECTORY/last-alignment-maf
