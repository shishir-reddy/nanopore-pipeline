...
###TBA###
# Case sensitive aligner pick 
# ADD IN INPUT FOR NAME + DATE OF TIME
# Add both option for aligner
# SPLIT EVERYTHING AFTER PORECHOP BY BARCODE
# Add date for nanoplot folder when scp'ing over
# Metapore
#########
...
# Aligner
echo "Enter Aligner to be used (LAST or minimap2)"
read ALIGNER
...
if [ "$ALIGNER" | tr '[:upper:]' '[:lower:]' = "last" ]; then
...
# MiniMap2 - Align to human genome using MiniMap2 
    ...
    mkdir $NANOSV_DIRECTORY
    module load NanoSV/1.1.2-foss-2016b-Python-3.6.4
    NanoSV -s samtools -c config.ini -b $RADICH_HOME/osala/nanopore/hg19-refFlat.bed -o $NANOSV_DIRECTORY/NanoSV.vcf $ALIGNMENT_DIRECTORY/minimap-alignment-sorted-bam
